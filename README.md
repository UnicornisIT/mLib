...
> **Статус:** Windows Desktop `0.0.1-alpha` и self-hosted server.
...
Готовая Windows-сборка устанавливается обычным файлом `mLib-Setup-0.0.1-alpha-x64.exe` и не требует Python, Node.js, Docker или отдельного сервера базы данных. Программа хранит SQLite-базы и media в `%LOCALAPPDATA%\mLib`, отдельно от EXE, и поддерживает Backup, Restore, переносимый Export и Import через `Настройки → Данные`.

Сборка для разработчика выполняется одной командой на Windows:

```powershell
.\scripts\build-windows.ps1
```

Подробности: [сборка desktop](docs/WINDOWS_DESKTOP_BUILD.md) и [руководство пользователя](docs/WINDOWS_DESKTOP_USER.md).

GitHub Actions автоматически публикует Windows EXE, контрольные суммы и updater-метаданные в `Releases` при отправке тега версии, например `v0.0.1-alpha`. Установленная alpha-версия проверяет новые предварительные Releases и предлагает скачать и установить обновление прямо внутри интерфейса. Ручной запуск workflow создаёт только тестовый artifact.
